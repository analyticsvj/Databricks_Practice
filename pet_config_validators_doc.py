...
from dsp.common.spark_helpers import database_exists

import pandas as pd

from datetime import datetime

def log_details(prefix: str, message: str):
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    print(f"[{timestamp}] [{prefix}] {message}")

def log_info(message: str):
    log_details(prefix="INFO", message=message)

def log_error(message: str):
    log_details(prefix="ERROR", message=message)

def log_warning(message: str):
    log_details(prefix="WARN", message=message)
    ...
    try:
        ...
        try:
            ...
            if not tables_df.filter(col("tableName") == table_name).count():
                log_warning(f"Table '{table_name}' not found in database '{db_name}'")
                return False
        except Exception as e:
            log_error(f"Failed to list tables in database '{db_name}': {e}")
            raise ValueError(f"Cannot access database '{db_name}': {e}")

        # Load schema
        try:
            full = f"{db_name}.{table_name}"
            log_info(f"Accessing table schema for: {full}")
            df_schema = spark.table(full).schema
        except Exception as e:
            em = str(e).lower()
            if ("table" in em) and ("not found" in em or "does not exist" in em):
                log_warning(f"Table '{full}' does not exist or is not accessible")
                return False
...
def validate_schedule_rules(param_data: dict) -> bool:
    ...
    log_info(f"Schedule validation passed for '{schedule}'")
    return True
...
def validate_dps_cdp_params(
    ...
    # Skip validations for Data-Provisioning stage
    if stage == "DATA-PROVISIONING":
        log_info("Stage is 'Data-Provisioning' - skipping Load Type, Incremental fields, and Schedule validations")
        return
...
def validate_and_normalize_incremental_timestamp_field(
    ...
    if not field_spec or not str(field_spec).strip():
        raise ValueError("Incremental Timestamp Field is missing - 1")
