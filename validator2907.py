...
def validate_schedule_rules(schedule: Optional[str], week_days: Optional[str], dates_of_month: Optional[str]) -> None:
    if not schedule:
        raise ValueError("Schedule is required and must be one of: DAILY, WEEKLY, MONTHLY")
    ...
    """
    Validates combinations of schedule, week_days, and dates_of_month according to defined rules:
    - DAILY: week_days and dates_of_month must be empty or null.
    - WEEKLY: week_days must be non-empty, dates_of_month must be null or empty.
    - MONTHLY: dates_of_month must be non-empty, week_days must be null or empty.
    """
    if not schedule:
        return
...
def validate_dps_cdp_params(spark: SparkSession, param_data: dict, db_name: str) -> bool:
    ...
    if load_type == "INCREMENTAL":
        ...
        validate_schedule_rules(schedule, week_days, dates_of_month)
...
#                 return current_map  # keep existing non-null value

#             return None  # record exists but last_process_datetime is null

#         # No record found, proceed with initialization
#         if not start_process_from_date_str:
#             return None
