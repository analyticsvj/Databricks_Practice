...
def fetch_xlsx_bytes(uri: str) -> bytes:
    ...
    if uri.lower().startswith("s3://"):
        ...
        log_info(f"PETFileLoader:Fetching bytes from s3://{bucket}/{key}")
    ...
    with open(path, "rb") as f:
        data = f.read()
    log_info(f"PETFileLoader:Fetched {len(data)} bytes from {uri}")
    return data
...
def write_config(
    ...
    try:
        ...
        log_info(f"Configuration written for dataset {dataset_id} (stage={stage}, overwrite={overwrite}).")
        return True

    except Exception as e:
        log_error(f"Failed to write to pet.ctrl_dataset_config: {e}")
        raise
...
def write_input_fields_table(spark: SparkSession, df: pd.DataFrame, dataset_id: str, stage: str) -> None:
    ...
    try:
        ...
        # Add explicit row order to preserve original Excel order
        data["row_order"] = range(len(data))

        spark_df = spark.createDataFrame(
            data[["dataset_id", "database_name", "table_name", "field_name", "comments", "stage", "row_order"]]
        )
...
#         )
        ...
        log_info(f"Input Schema written for dataset {dataset_id} (stage={stage}).")
    except Exception as e:
        log_error(f"Failed to write to pet.ctrl_dataset_input_fields: {e}")
        raise
...
class PETFileLoader:
    ...
    def LoadConfigurationsForDataset(
        ...
        try:
            ...
            log_error(f"Error during config load: {e}")
